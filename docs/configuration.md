...
| `exchange.incremental_ohlcv_refresh` | Reuse the cached candles and ask the exchange only for candles newer than the last refresh. Greatly decreases candle download latency/bandwidth. <br>*Defaults to `true`.* <br> **Datatype:** Boolean
| `exchange.incremental_ohlcv_lookback` | How many completed candles to re-request together with the new candle when incremental refresh is enabled. Higher values add redundancy at the cost of slightly more data. <br>*Defaults to `2`.* <br> **Datatype:** Positive Integer
