...
def check_exchange(config: Config, check_for_bad: bool = True) -> bool:
    ...
    if mapped_exchange not in NON_CCXT_EXCHANGES:
        ...
        logger.info(
            f'Exchange "{exchange}" uses a native implementation and is not part of the ccxt list.'
        )

    if mapped_exchange in SUPPORTED_EXCHANGES:
        logger.info(
            f'Exchange "{exchange}" is officially supported by the Freqtrade development team.'
        )
    else:
        if mapped_exchange in NON_CCXT_EXCHANGES:
            logger.warning(
                f'Exchange "{exchange}" uses a native implementation and is not officially '
